...
%% Right
\node[font=\small\sffamily\bfseries\color{kblue!80}] at (8.72,6.42) {Godspeed Subscales};
\draw[kblue!30, line width=0.3pt] (3.65,6.10) -- (13.8,6.10);
\fill[kblue!10, rounded corners=3pt] (3.65,5.20) rectangle (13.8,6.00);
\node[font=\scriptsize\sffamily\color{kblue!85}, anchor=west, text width=9.65cm]
  at (3.80,5.60) {\textbf{Five subscales} (5 items, 5-point each): Anthropomorphism, Animacy, Likeability, Perceived Intelligence, Perceived Safety};
\fill[kblue!10, rounded corners=3pt] (3.65,4.35) rectangle (13.8,5.15);
\node[font=\scriptsize\sffamily\color{kblue!85}, anchor=west, text width=9.65cm]
  at (3.80,4.75) {\textbf{Likeability} (Part B): affective warmth and appeal --- proxy for social acceptance and the affective component of trust};
\fill[kblue!10, rounded corners=3pt] (3.65,3.50) rectangle (13.8,4.30);
\node[font=\scriptsize\sffamily\color{kblue!85}, anchor=west, text width=9.65cm]
  at (3.80,3.90) {\textbf{Anthropomorphism} (Part C): perceived human-likeness --- does NPR reduce eeriness while preserving human-likeness?};
\fill[kblue!10, rounded corners=3pt] (3.65,2.65) rectangle (13.8,3.45);
\node[font=\scriptsize\sffamily\color{kblue!85}, anchor=west, text width=9.65cm]
  at (3.80,3.05) {\textbf{We do NOT use:} animacy, perceived intelligence, or perceived safety --- outside this study's scope, adds measurement fatigue};
\fill[kgreen!12, draw=kgreen!40, rounded corners=3pt, line width=0.6pt]
  (3.65,0.1) rectangle (13.8,2.50);
\node[font=\small\sffamily\bfseries\color{kgreen!70!black}, anchor=west]
  at (3.82,2.20) {What this study adopts};
\node[font=\footnotesize\sffamily\color{kgreen!80!black}, anchor=north west, text width=9.65cm]
  at (3.82,2.05) {Part B --- 5 Likeability items (5-point). Part C --- 5 Anthropomorphism items (5-point). Anthropomorphism tests whether NPR mediates eeriness by changing perceived humanness.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=10.5cm, anchor=north west]
    at (0.72,5.87)
    {You have just arrived by train in an unfamiliar city and must reach your hotel
     in about one hour for a colleague meeting. Your map app shows two routes.};
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=4.7cm, anchor=north west]
    at (0.75,4.45)
    {20 min with one transfer at an unfamiliar station.
     Arrives \textbf{40 min early} if the connection holds.};
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=4.7cm, anchor=north west]
    at (6.10,4.57)
    {50 min, single direct line, no transfers.
     Arrives \textbf{10 min before} the meeting. Reliable.};
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=10.5cm, anchor=north west]
    at (0.72,5.87)
    {You want to learn data analysis to strengthen your CV. You have a few weeks
     available and have found two options to choose between.};
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=4.7cm, anchor=north west]
    at (0.75,4.45)
    {Structured online course. Costs 35 euros, approx.\ 8 hours.
     \textbf{Awards a certificate} from a named platform.};
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=4.7cm, anchor=north west]
    at (6.10,4.57)
    {Free tutorial series by a practitioner with a strong reputation.
     Same material, flexible pace. \textbf{No certificate.}};
...
\begin{frame}{Scene 6 --- Part D: Eeriness \textnormal{\small (Ho \& MacDorman, 2010 --- 6 items, 5-point)}}
\begin{center}
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  %% Sem-diff items (Part D — Eeriness, 6 items from Ho & MacDorman 2010)
  \foreach \y/\itnum/\la/\ra in {
    5.30/D1/{Not at all eerie}/{Very eerie},
    4.40/D2/{Not at all creepy}/{Very creepy},
    3.50/D3/{Predictable}/{Unsettling},
    2.60/D4/{Ordinary}/{Supernatural},
    1.70/D5/{Comfortable to watch}/{Disturbing to watch},
    0.80/D6/{Familiar}/{Feels out of place}
  }{
    \fill[kbluepale!80] (0.0,{\y-0.38}) rectangle (14.0,{\y+0.38});
    \draw[kblue!15, line width=0.3pt] (0.0,{\y-0.38}) -- (14.0,{\y-0.38});
    \node[font=\footnotesize\sffamily\bfseries\color{kblue}, anchor=west] at (0.1,\y) {\itnum};
    \node[font=\footnotesize\sffamily\itshape\color{kgray}, anchor=east, text width=4.8cm, align=right]
      at (5.80,\y) {\la};
    \foreach \i in {0,1,2,3,4}{
      \draw[kblue!50, line width=0.9pt] ({6.18+\i*0.65},\y) circle (0.15);
    }
    \node[font=\footnotesize\sffamily\itshape\color{kgray}, anchor=west, text width=4.8cm]
      at (8.92,\y) {\ra};
  }
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[font=\footnotesize\sffamily\color{kblue}, text width=13cm, align=center] at (7.0,6.10)
    {Imagine each advisor advising on \textbf{any} topic --- not only the scenario you saw. Rank from \textbf{most} (1) to \textbf{least} (3) preferred to consult. Assign each advisor a different rank.};
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[font=\footnotesize\sffamily\bfseries\color{kblue}, text width=13cm, align=center, anchor=north]
    at (7.0,2.02) {P4. Which advisor suits a \textbf{professional context} most?};
...
\begin{tikzpicture}[x=1cm, y=1cm]
  ...
  \node[font=\footnotesize\sffamily\bfseries\color{kblue}, text width=12.0cm, anchor=north west] at (0.7,3.52)
    {J. \ Was there a specific moment, comment, or visual detail in any of the three videos that noticeably increased or decreased your trust in that advisor? If yes, please describe briefly.};
  ...
  \node[font=\tiny\sffamily\color{kgray}, text width=12cm, align=center] at (6.8,1.30)
    {P5 and P6 are open-ended and optional. J is post-study and covers all three videos. Responses supplement the quantitative scales.};
