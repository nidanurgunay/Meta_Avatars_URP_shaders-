...
\vspace{8pt}
\subsection{Dill et al.\ (2012) --- Evaluation of the Uncanny Valley in CG Characters}

\begin{refbox}
\textbf{Full reference:} Dill, V., Flach, L. M., Hocevar, R., Lykawka, C., Musse, S. R., \& Pinho, M. S. (2012). Evaluation of the uncanny valley in CG characters. In Y. Nakano, M. Neff, A. Paiva, \& M. Walker (Eds.), \textit{Intelligent Virtual Agents: 12th International Conference, IVA 2012}, Lecture Notes in Artificial Intelligence, vol.\ 7502, pp.\ 511--513. Springer, Heidelberg.\\[4pt]
\textbf{Note:} Poster paper (3 pages). Proceedings DOI: \texttt{10.1007/978-3-642-33197-8}
\end{refbox}

\textbf{Research question.} Does the uncanny valley effect, originally proposed by Mori (1970) for robots and androids, extend to contemporary CG characters from movies and games? The paper also asks whether adding motion to these characters changes the shape of the uncanny valley curve.

\textbf{Methodology.} The authors selected ten representative characters from recent and well-known movies and games spanning a range of human-likeness. A two-stage questionnaire was administered to participants. In the first stage, participants rated still images of the characters; in the second stage, they rated video clips of the same characters performing the same scenes. Ratings were plotted with human-likeness on the horizontal axis and familiarity level on the vertical axis, directly replicating Mori's original graph format. Participants were also asked which parts of the face caused the most discomfort where any was experienced. The exact sample size is not reported in this poster contribution.

\textbf{Results.} The obtained data produced a chart closely following the original uncanny valley curve proposed by Mori. Characters in the near-human range (C.11, C.16, C.1, and C.14) showed the characteristic negative response: they were rated as highly human-like but received low familiarity scores, consistent with the ``almost human but unnatural'' pattern. The curves for still images and for video were both coherent with Mori's original. The video curve showed more pronounced valley effects for near-human characters than the still-image curve, consistent with Mori's prediction that movement amplifies the uncanny response. When asked which facial features caused discomfort, participants consistently identified two regions: the \textbf{eyes} and the \textbf{mouth}.

\textbf{Limitations.} This is a three-page poster paper; no statistical tests are reported and the sample size is not stated. The ten characters were selected from existing productions rather than systematically placed along a controlled realism continuum, meaning the spacing of realism levels is uneven. No demographic data (age, gender, familiarity with animated films or games) were collected or controlled.

\textbf{Relevance to this thesis.} This paper is one of the few empirical validations of the uncanny valley effect for CG characters from contemporary digital media, rather than for robots or androids. It confirms that the phenomenon is not confined to physical hardware and extends to the type of content this thesis works with. The identification of eyes and mouth as the primary sources of facial discomfort is directly relevant to avatar design: these are the most expressive and detail-dependent features of a face, and NPR stylisation that abstracts skin texture and smooths micro-detail in these regions is expected to reduce precisely this discomfort. The replication of the valley for both still images and video supports the thesis's video-based study design, and the motion amplification result provides additional justification for using dynamic stimuli. The paper provides direct empirical grounding for H1 and for the overall motivation that photorealistic avatars risk entering the uncanny zone in a way that NPR stylisation can mitigate.
...
\vspace{8pt}
\subsection{Wallraven, Breidt, Cunningham \& Bülthoff (2008) --- Evaluating the Perceptual Realism of Animated Facial Expressions}

\begin{refbox}
\textbf{Full reference:} Wallraven, C., Breidt, M., Cunningham, D. W., \& Bülthoff, H. H. (2008). Evaluating the perceptual realism of animated facial expressions. \textit{ACM Transactions on Applied Perception}, 4(4), Article 23, 20 pages.\\[4pt]
\textbf{DOI:} \texttt{10.1145/1278760.1278764}
\end{refbox}

\textbf{Background and motivation.} Creating believable animated CG faces remains one of the most challenging tasks in computer graphics. The challenge of evaluating quality is equally hard: most prior work relied on explicit judgments such as ``does this look real?'', which capture only a narrow slice of perceptual experience. Wallraven et al.\ propose an integrated psychophysical framework that replaces explicit realism judgments with task-dependent perceptual measures, benchmarked against real video of the same actor performing the same expressions.

\textbf{Method: three experiments.}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Experiment 1 (animation technique comparison).} Six animation conditions were compared: a full hybrid avatar (motion capture timing + high-resolution 3D scan shape), avatar without rigid head motion, linear morph, cluster animation, 4D dynamic scan, and static 4D peak frame. All expressed seven emotions (confusion, disgust, fear, happy, sad, surprise, thinking). Participants ($n = 12$) rated each stimulus on four perceptual measures: recognition accuracy, reaction time, perceived intensity, sincerity, and typicality.
  \item \textbf{Experiment 2 (information channels).} Using the benchmarked avatar as baseline, the three information channels of shape (surface normals), texture (albedo), and motion were each independently degraded using Gaussian blur across four blur levels. The experiment assessed how each channel contributes to the perceptual measures.
  \item \textbf{Experiment 3 (eyes and teeth).} The avatar lacked eyes in the first two experiments. Eyes (two spherical eye meshes with constrained gaze-at-camera motion) and teeth were added, and perceptual measures were re-collected for both static and dynamic conditions.
\end{itemize}

\textbf{Key findings.}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Avatar achieves comparable recognition to video.} The full avatar condition reached 65.5\% recognition accuracy, matching real-world video benchmarks for most expressions (with exceptions: ``confusion'' and ``thinking'').
  \item \textbf{Rigid head motion is critical.} Removing head motion alone (AvaNoR condition) dropped recognition to 44.2\% --- a loss of 21 percentage points. Adding head motion back restored performance and significantly reduced reaction time.
  \item \textbf{Motion dominates over shape and texture.} In Experiment 2, degrading shape or texture had no significant effect on recognition as long as motion was intact. When motion was removed (static peak frame), shape degradation severely reduced recognition. Temporal information can compensate for degraded static channels, but not the reverse.
  \item \textbf{Intensity and sincerity require motion.} Blurring either shape or motion significantly reduced perceived intensity and sincerity ratings, even when recognition was unaffected. This demonstrates that recognition and quality are not the same perceptual outcome.
  \item \textbf{Eyes significantly improve recognition and quality.} Adding eyes produced a gain of approximately 5\% in recognition accuracy ($p < 0.001$) and significantly increased intensity and sincerity for most expressions. The benefit was larger for dynamic sequences than static frames.
\end{itemize}

\textbf{Key citable sentences (original text and location).}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Section 2.2, p.\ 23:4:} ``Having access to an indirect, perceptual measure is especially important in the field of human–machine communication, which requires, for example, avatars for virtual kiosks or sales tasks [...] For these applications it is crucial that the facial information displayed by the avatar should not only be recognizable, but also believable and sincere.''
  \item \textbf{Section 3, p.\ 23:5:} ``Computer-animated expressions should be recognized as quickly as human expressions are. Any human–machine interaction will be strongly and negatively affected if it takes a person too long to decipher the computer animated expressions.''
  \item \textbf{Section 3.2.3 Summary, p.\ 23:11:} ``Both types of temporal information ensure that the resulting facial expressions are recognized reliably and quickly, as well as providing important cues for judging their intensity, sincerity, and typicality.''
  \item \textbf{Section 3.3.3 Summary, p.\ 23:14:} ``Interestingly, blurring the shape and texture channels had no effect on recognition rates, as long as the motion channel was present, although it did increase reaction time in both conditions.''
  \item \textbf{Section 3.4, p.\ 23:14:} ``One of the biggest drawbacks, in terms of realism and perceptual plausibility, of the avatar as it was used in the two previous experiments is its lack of eyes and teeth. It is well known, for example, that humans are very sensitive to eye motion.''
  \item \textbf{Section 4 (Summary), p.\ 23:18:} ``Whereas the ultimate goal of computer graphics usually has been to achieve maximum physical realism, perceptual measures seem better suited to capture success or failure of animated facial expressions.''
  \item \textbf{Section 4 (Summary), p.\ 23:19:} ``Taken together, our experimental results shed light on how humans use different information channels to evaluate various aspects of facial expressions --- results that are not only of interest for perception research but also for computer animation (including the use of avatars in virtual reality and computer-supported collaborative work).''
\end{itemize}

\textbf{Relevance to this thesis.} This paper motivates two key design choices in the user study. First, the argument that recognition alone is insufficient to measure avatar quality directly supports the multi-component questionnaire design used here, which collects eeriness, likeability, trust, and sincerity ratings alongside the primary behavioural measure. A CG avatar that is recognised correctly may still trigger negative responses along other perceptual dimensions, as Wallraven et al.\ demonstrate for intensity and sincerity. Second, the finding that motion is the dominant information channel for both recognition and quality ratings supports the choice of video stimuli rather than static images: all three conditions (C1 real, C2 photorealistic avatar, C3 NPR avatar) present dynamic facial motion, ensuring the dominant channel is preserved across the comparison. The concept of ``perceptual equivalence'' --- that the goal is to match the human benchmark on perceptual measures rather than on physical fidelity --- is directly applicable to framing the comparison between C2 and C3: the question is not whether C3 looks objectively less detailed, but whether it is perceptually equivalent to C2 on the measures of trust and eeriness that the study targets.

\textbf{Exact sentence cited in thesis.} The thesis claim that reducing shape detail degrades expressive readability cites \texttt{wallraven2008} using the following sentence from \S4 Summary (p.\ 23:19): \textit{``In addition, in Experiment~2, we found clear effects of shape blurring on both intensity and sincerity judgments. This demonstrates that, even though reduced shape information does not affect recognition, it does influence the perceived quality of the expression.''}
...
\vspace{8pt}
\subsection{Zell et al.\ (2015) --- To Stylize or Not to Stylize? Shape and Material Stylization on CG Face Perception}

\begin{refbox}
\textbf{Full reference:} Zell, E., Aliaga, C., Jarabo, A., Zibrek, K., Gutierrez, D., McDonnell, R., \& Botsch, M. (2015). To stylize or not to stylize? The effect of shape and material stylization on the perception of computer-generated faces. \textit{ACM Transactions on Graphics}, 34(6), Article 184.\\[4pt]
\textbf{DOI:} \texttt{10.1145/2816795.2818126}
\end{refbox}

\textbf{Research question.} When designing a stylised 3D character, which parameters matter most for how observers perceive the character? Zell et al.\ identified two primary appearance variables: \textit{shape} (the overall geometric form, ranging from highly stylised cartoon geometry to realistic scanned head geometry) and \textit{material} (the combination of shader model, shading parameters, and albedo texture, spanning the same cartoon-to-realistic range). The study asks how varying each of these two dimensions independently, and in combination, affects perceived realism, appeal, eeriness, familiarity, and attractiveness.

\textbf{Methodology.} Three experiments were conducted. Characters were based on 3D scans of a male and a female actor, from which professional 3D artists produced stylised versions at multiple levels inspired by commercial animation films --- specifically \textit{Cloudy with a Chance of Meatballs} and \textit{Toy Story} for the stylised levels, and \textit{Beowulf} for the realistic level. Experiment 1a crossed three shape stylisation levels with three material levels (nine combinations) across four facial expressions in static renders. Experiment 1b tested shading and lighting parameters independently of texture. Experiment 1c tested blurred-texture variants of the realistic albedo to isolate the role of skin impurity detail. Experiment 2 extended to five shape levels and five material levels (25 combinations) across two characters and five expressions, for a total of 250 stimuli. Experiment 3 measured how stylisation affects expression recognition and perceived emotional intensity. All ratings used seven-point Likert scales; statistical analysis used repeated-measures ANOVA with Greenhouse-Geisser correction and Tukey HSD post-hoc tests. Sample sizes ranged from 21 to 24 participants per experiment.

\textbf{Finding 1: Shape governs realism; material governs appeal and eeriness.} A consistent dissociation emerged across all experiments. Shape was the dominant factor for perceived realism: increasing shape realism reliably increased realism ratings. Material was the dominant factor for appeal, attractiveness, and eeriness: realistic materials were the least appealing and most eerie, while cartoon materials were consistently more appealing across all shape levels. The realism scale and the appeal scale were not significantly correlated ($\alpha < 0.5$ in all experiments), confirming that these two dimensions are independent perceptual axes.

\textbf{Finding 2: Mismatch between shape and material is costly.} When shape and material stylisation levels were mismatched, appeal and attractiveness dropped significantly below any matching combination, and eeriness increased. The most unappealing and eerie stimulus across all experiments was a realistic material applied to the most stylised cartoon shape. Matching the stylisation degree of shape and material maximised appeal regardless of the absolute level of realism in either dimension. This pattern generalised across both characters and all five expressions tested.

\textbf{Finding 3: Realism is a bad predictor for appeal.} The realism-versus-appeal space (Figure 12 in the paper) shows all shape-level data points dispersed across a wide range of appeal values at any given realism level. A character can be rated high on realism and low on appeal, or moderate on realism and high on appeal. The data are consistent with the uncanny valley (Mori, 1970) specifically for the most realistic shape and material combination, but the broader picture is that it is the \textit{compatibility} of shape and material, rather than absolute realism, that determines where a character lands on the appeal curve.

\textbf{Finding 4: Subtle material stylisation increases appeal without sacrificing realism.} In Experiment 1c, applying a Gaussian blur of 50 pixels to the realistic albedo texture significantly increased appeal and attractiveness ($p < 0.0002$) while leaving realism ratings statistically unchanged. This effect was strongest on cartoon and middle shapes and was not significant for the realistic shape, suggesting that texture simplification raises appeal specifically in the mid-range of stylisation where a realistic texture creates the greatest mismatch with simplified shape geometry.

\textbf{Finding 5: Shape controls expression intensity; material does not.} In Experiment 3, perceived expression intensity decreased continuously with increasing shape stylisation: more stylised shapes conveyed lower emotional intensity even when the expression was correctly recognised. Material stylisation had no significant effect on perceived expression intensity. Expression recognition accuracy nonetheless remained high across all stylisation levels for most emotions, confirming that stylised characters remain readable for basic emotional communication.

\textbf{Limitations.} The study used static images rather than animated characters. Character models were based on two specific individuals; the generalisability to very different face morphologies is uncertain. Stylisation levels were defined by artist interpretation of specific commercial animation reference films and may not map cleanly onto abstract stylisation scales used in other research.

\textbf{Relevance to this thesis.} This paper is the most directly applicable empirical precedent for the material-rendering NPR strategy used in this project. The thesis holds avatar shape constant across all three conditions and changes only the material rendering through NPR shaders. Zell et al.\ demonstrate that material is precisely the dimension governing appeal and eeriness independently of shape realism, meaning the NPR shader pipeline operates in the perceptual domain with the strongest effect on the outcome variables measured in the user study. The finding that mismatched stylisation raises eeriness also validates the design decision to apply a consistent NPR treatment across the full avatar surface rather than selectively stylising only skin or only clothing. The result from Experiment 1c, that moderate material stylisation increases appeal without degrading realism, provides direct empirical support for the prediction in H1 that the NPR condition (C3) will produce lower eeriness ratings than the photorealistic condition (C2).

\textbf{Key citable sentences (original text and location).}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Abstract / Main Findings, p.\ 184:1:} ``Our experiments reveal that shape is the dominant factor when rating realism and expression intensity, while material is the key component for appeal. Furthermore our results show that realism alone is a bad predictor for appeal, eeriness, or attractiveness.''
  \item \textbf{Main Findings list, p.\ 184:2:} ``The perceived intensity of expressions decreases with realism of shape, but is nearly independent of material stylization.''
  \item \textbf{Main Findings list, p.\ 184:2:} ``On the other hand, appeal, eeriness, and attractiveness are mainly affected by the stylization level of material, and not shape; realistic materials reduce appeal in general.''
  \item \textbf{Main Findings list, p.\ 184:2:} ``The degree of realism is a bad predictor for appeal or eeriness.''
\end{itemize}

\textbf{Exact sentence cited in thesis.} The thesis claim that excessive stylisation undermines expressive readability cites \texttt{zell2015} using the following sentence from the Main Findings list (p.\ 184:2): \textit{``The perceived intensity of expressions decreases with realism of shape, but is nearly independent of material stylization.''} (Interpretation: as shape stylisation increases and realism decreases, perceived expression intensity also decreases --- more stylised shapes convey lower emotional intensity.)
...
\noindent{\small\sffamily Summaries sourced from detailed reading of referenced articles and written by Nidanur G\"{u}nay, University of Konstanz, June 2026. \textbf{Uncanny Valley} section covers Mori (1970), MacDorman \& Ishiguro (2006), Ho \& MacDorman (2010), McDonnell et al.\ (2012), Dill et al.\ (2012), and Seymour et al.\ (2021). \textbf{Trust and Avatar Perception} section covers McKnight \& Chervany (2001), Mayer et al.\ (1995), Bartneck et al.\ (2009), Canales et al.\ (2024), Alipour et al.\ (2025), Nowak \& Biocca (2003), Nowak \& Rauh (2005), Weidner et al.\ (2023), Kuffner dos Anjos \& Pereira (2024), Alimardani et al.\ (2024), Dubosc et al.\ (2025), Tao et al.\ (2025), Cihodaru-\c{S}tefanache \& Podina (2025), and Wallraven et al.\ (2008). \textbf{NPR} section covers Gooch et al.\ (1998), Lake et al.\ (2000), Isenberg et al.\ (2003), Gonzalez \& Woods (2018), Marr \& Hildreth (1980), Canny (1986), Barla et al.\ (2006), Kyprianidis et al.\ (2009), Praun et al.\ (2001), Roberts (1963), Kumar \& Poornima (2019), Wisessing et al.\ (2016), Wisessing et al.\ (2020), Petikam et al.\ (2021), Riefard et al.\ (2024), Zell et al.\ (2015), Roshaan (2026, preprint), [Author et al.]\ (2024, VR Rehabilitation), and [Microsoft Research Authors]\ (2025, MR Longitudinal).}
